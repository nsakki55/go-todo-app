...
import (
	"context"
	"github.com/nsakki55/go-todo-app/testutil/fixture"
	"testing"
...
func prepareTasks(ctx context.Context, t *testing.T, con Execer) (entity.UserID, entity.Tasks) {
	...
	result, err := con.ExecContext(ctx,
		...
		tasks[0].UserID, tasks[1].Title, tasks[1].Status, tasks[1].Created, tasks[1].Modified,
		tasks[0].UserID, tasks[2].Title, tasks[2].Status, tasks[2].Created, tasks[2].Modified,
	)
	if err != nil {
		t.Fatal(err)
	}
